...
const fn = async (conversation) => {
    ...
    // ];
    const response = await g4f.chatCompletion(conversation)
    return response
}

// g4f.translation(options2).then(console.log)

app.post("/edgarvilchez", async (req,res) => {
    const {conversation} = req.body
    const response = await fn(conversation)
    res.json(response)
})
